...
template <class NumericType, int D> class AtomicLayerProcess {
...
public:
  AtomicLayerProcess(
      const SmartPointer<DenseCellSet<NumericType, D>> &passedCellSet,
      const bool passedEtch = false)
      : cellSet(passedCellSet), etch(passedEtch),
  ...
  void apply() {
    ...
    if (cellType == nullptr) {
      Logger::getInstance().addError("CellType scalar data not found.").print();
    }
    ...
    while (time < precursors[0].duration) {
      ...
      if (time - i * printInterval > 0. && Logger::getLogLevel() > 3) {
    ...
    while (time < purge_duration) {
      ...
      if (time - j * printInterval > 0. && Logger::getLogLevel() > 3) {
        cellSet->writeVTU(fileName + std::to_string(i++) + ".vtu");
        ++j;
      }
    }
    ...
    while (time < precursors[1].duration) {
      ...
      if (time - k * printInterval > 0. && Logger::getLogLevel() > 3) {
        cellSet->writeVTU(fileName + std::to_string(i++) + ".vtu");
        ++k;
      }
    }
